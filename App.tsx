...
// SERVICES
import { twilioService as mockTwilioService } from './services/mockTwilio';
...
// Set this to FALSE to use the in-browser demo mode (no backend required)
...
const App: React.FC = () => {
  ...
  // Select the appropriate service based on config
  const activeTwilioService = USE_BACKEND ? liveTwilioService : mockTwilioService;
    ...
    reader.onload = async (e) => {
      ...
      if (text) {
        ...
        if (newProspects.length > 0) {
          if (USE_BACKEND) {
             // Bulk create in backend (sequentially for simplicity here)
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'power-dialer':
        return <PowerDialer queue={powerDialerQueue} onCall={handleCall} />;
      case 'manual-dialer':
        return <ManualDialer onCall={handleManualCall} />;
      case 'history':
      ...
      case 'settings':
        return (
          <Settings 
            ...
            // In backend mode, use the API directly to fetch numbers
            // In mock mode, Settings component handles it internally via mockTwilio
          />
        );
      default:
